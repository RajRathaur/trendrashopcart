...
export const GamePromoBanner = () => {
  const highScore = parseInt(localStorage.getItem('coinGameHighScore') || '0');

  return (
    <section className="py-3">
      <Link to="/coin-game" className="block">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="relative overflow-hidden rounded-xl bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 p-5 md:p-6"
        >
          ...
          <div className="relative z-10 flex items-center gap-4">
            <div className="flex-shrink-0 w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
              <Gamepad2 className="w-7 h-7 md:w-8 md:h-8 text-white" />
            </div>

            <div className="flex-1 min-w-0">
              <h3 className="text-white font-bold text-lg md:text-xl">🎮 Coin Catcher</h3>
              <p className="text-white/80 text-xs md:text-sm mt-0.5">
                Catch coins, dodge bombs & win rewards!
              </p>
              {highScore > 0 && (
            ...
            <Button size="sm" className="flex-shrink-0 bg-white text-purple-700 hover:bg-white/90 font-bold gap-1 shadow-lg">
